...
#         "type": msg_type
#     })
def add_message(role: str, content: str, msg_type: str = "message", result: dict = None):
    st.session_state.messages.append({
        "role": role,
        "content": content,
        "type": msg_type,
...
for i, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        ...
        if msg["role"] == "assistant" and msg.get("result"):
            render_agent_output_inline(
                result=msg["result"],
                key_prefix=f"chat_{i}"
            )
# =============================
# CHAT INPUT
# =============================
        ...
        with st.spinner("Thinking..."):
            try:
                ...
                add_message(
                    ...
                    msg_type="clarification" if result.get("waiting_for_user") else "answer",
                    result=result
                )
            except Exception as e:
